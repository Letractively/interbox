 ...
 * @package interbox.core1.resource.filesystem
 */
...
class ResourceManager extends ServiceManager {
    ...
    public function Create($ServiceName, $UserService, $Root, $FileTypeList="txt htm html gif jpg png bmp rar zip", $MaxFileSize=1048576) {
        ...
        if (!is_dir($Root)) {
            ...
            //$this->GetError()->AddItem(4,"|the root does not exist");
            //return FALSE;
        }
        if (!$conn->TableExists("ibc1_dataservice_resource")) {
            ...
            if (!$this->CreateTables($sqlset, $conn)) {
                $this->GetError()->AddItem(3, "Resource服务建立失败|fail to create a Resource service");
                return FALSE;
            }
        }
        ...
        if ($r == FALSE) {
            $this->GetError()->AddItem(3, "Resource服务建立失败|fail to create Resource service");
            return FALSE;
        }
