...
if (GetQueryString("mode") == "download") {
    LoadIBC1Class("FileItemReader", "resource.database");
    $fir = new FileItemReader($conn, "resourcedbtest");
...
<html>
    <head>
        <title>InterBox Core 1.1.4 [For PHP] Resource.Database</title>
    </head>
    <body>
        <?php
        ...
        if (GetQueryString("mode") == "getupload") {
            LoadIBC1Class("FileUploader", "resource.database");
            $fu = new FileUploader($conn, "resourcedbtest");
            ...
            LoadIBC1Class("FileItemEditor", "resource.database");
            $fie = new FileItemEditor($conn, "resourcedbtest");
            $fie->Open($id);
            $r = $fie->Delete();
            showResult($r);
        }
        ...
        <?php
        LoadIBC1Class("FileListReader", "resource.database");
        $flr = new FileListReader($conn, "resourcedbtest");
        $flr->LoadList();
        $c = $flr->Count();
        if ($c > 0) {
            echo "<table border=1>";
            for ($i = 0; $i < $c; $i++) {
                ...
                echo "<td><a target=\"_blank\" href=\"test_resource_db.php?mode=download&id=" . $flr->GetItem($i)->filID . "\">download</a></td>";
                echo "<td><a href=\"test_resource_db.php?mode=delete&id=" . $flr->GetItem($i)->filID . "\">delete</a></td>";
                echo "</tr>";
            }
            echo "</table>";
        }
        ...
        <form enctype="multipart/form-data" name="UploadForm" method="post"
              action="test_resource_db.php?mode=getupload">
            <table>
                <tr>
                    <td align="center">
                        <table>
